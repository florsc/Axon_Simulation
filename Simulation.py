...
class Simulation:
  ...
  def sampleModelGrowthVector(self,growthStepThisTimePoint):
    ...
    thetas_0.append(np.random.normal(expectation_0,variation_0)); #this modifies self.thetas

    thetas_1 = self.thetasThisTimePoint[1]
    expectation_1 = (alpha/(alpha+beta))*thetas_1[step-1]
    variation_1 = 1/math.sqrt(2*(alpha+beta))
    thetas_1.append(np.random.normal(expectation_1,variation_1)); #this modifies self.thetas
  ...
  def runTimeStepForAxon(self,currentAxon):
    self.thetasThisTimePoint = [[currentAxon["lastTheta"][0]],[currentAxon["lastTheta"][1]]]
    growthStepThisTimePoint = 1


    while growthStepThisTimePoint < self.staticParameters["maximumNumberOfStepsEachTimePoint"]:
        growthVector = self.sampleModelGrowthVector(growthStepThisTimePoint)
        currentAxon["axonalTipPositions"].append(growthVector + currentAxon["axonalTipPositions"][-1]) #TODO this could cause issues when retraction is implemented, replace -1
        growthStepThisTimePoint += 1
  ...
  def getAxisStartPositions(self,axisIndex):
    if self.staticParameters["startingArea"][axisIndex][0] == self.staticParameters["startingArea"][axisIndex][1]:
        ...
        spacePerAxon = self.staticParameters["axonDiameter"]*spatialStartingCubeFactor
        axisLength = self.staticParameters["startingArea"][axisIndex][1]-self.staticParameters["startingArea"][axisIndex][0]
        numberOfPositions = math.floor(axisLength/spacePerAxon)
        remainingSpaceAtBorder = (axisLength-(numberOfPositions*spacePerAxon))/2
        axisPositions = [remainingSpaceAtBorder + (p * spacePerAxon) for p in range(numberOfPositions)]
  ...
  def getStartPositions(self):
    ...
    numberOfPositions = len(xAxisPositions) * len(yAxisPositions) * len(zAxisPositions)
    ...
    linearPositionIndices = np.random.choice(numberOfPositions,self.staticParameters["startingNumberOfAxons"], replace = False)
    lenX = len(xAxisPositions)
    lenY = len(yAxisPositions)
    lenZ = len(zAxisPositions)
    xPositionsIndices = [math.floor(linearPositionIndices[i] /(lenY*lenZ)) for i in range(len(linearPositionIndices))]
    yPositionsIndices = [math.floor((linearPositionIndices[i] - (lenX * xPositionsIndices[i])) / lenZ) for i in range(len(linearPositionIndices))]
    zPositionsIndices = [math.floor((linearPositionIndices[i] - (lenX * xPositionsIndices[i]) - (lenY * yPositionsIndices[i]))) for i in range(len(linearPositionIndices))]
    positions = [[xAxisPositions[xPositionsIndices[i]], yAxisPositions[yPositionsIndices[i]], zAxisPositions[zPositionsIndices[i]]] for i in range(self.staticParameters["startingNumberOfAxons"])]
    return positions
  ...
  def runSimulation(self):
    while (self.numberOfAxons - self.numberOfFinishedAxons) > 0:
        ...
        for axon in self.numberOfAxons:
            ...
            self.numberOfFinishedAxons+=0.1 #will be removed, currently only to ensure ending of loop

  def visualizeResults(self):
    #currently simple visualization for the first axon to check result
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    xline = [tipPosition[0] for tipPosition in self.axons[0]["axonalTipPositions"]]
    yline = [tipPosition[1] for tipPosition in self.axons[0]["axonalTipPositions"]]
